...
for x in 0..3
	...
	browser.goto 'http://[::1]/HRDO_SAFS/admin/employeelist/123456789'
	...
	# date of birth
	if x == 0 or x == 2
		browser.text_field(:id => 'birthdate').set date_birth[0]
	elsif x == 1 or x == 3
		browser.text_field(:id => 'birthdate').set date_birth[1]
		date_birth_flag = 1
	end
	

	# gender
	browser.div(:id => "gender_text").click
	if x == 0 or x == 3
	...
	# unit
	browser.div(:id => "unit_text").click
	if x == 0
	...
	# department
	browser.div(:id => "department_text").click
	if x == 0
	...
	# employee type
	browser.div(:id => 'employee_text').click
	if x == 0 or x == 3
	...
	# employment status
	browser.div(:id => 'status_text').click
		if x == 0 
	...
	#rank
	browser.div(:id => 'rank_text').click
	if x == 0 
